...
export default function DiasPendentes({ id, cargo }) {
    ...
    const [gestorSelecionado, setGestorSelecionado] = useState(''); // Novo estado do Gestor
    const [gestoresDisponiveis, setGestoresDisponiveis] = useState([]); // Lista para cargo MASTER
    ...
    // 1. CARREGA GESTORES (SOMENTE SE FOR MASTER)
    useEffect(() => {
        const carregarGestores = async () => {
            if (!isMaster) return;
            try {
                // Rota que retorna todos os gestores/usuários ativos do sistema
                const res = await axios.get(`${API_URL}/gestor/lista-gestores`);
                setGestoresDisponiveis(res.data || []);
            } catch (e) {
    ...
    // 2. CARREGA AS OBRAS ATIVAS (DEPENDENDO DO GESTOR SELECIONADO OU DO USUÁRIO LOGADO)
    const carregarObras = useCallback(async () => {
        try {
            const res = await axios.get(`${API_URL}/gestor/obras-ativas`, {
                params: { 
                    id: isMaster && gestorSelecionado ? gestorSelecionado : id, 
                    cargo: isMaster && !gestorSelecionado ? 'MASTER' : cargo 
                }
            });
    ...
    }, [id, cargo, isMaster, gestorSelecionado]);
    ...
    // 3. EXECUTA A AUDITORIA CRUZANDO CALENDÁRIO COM DADOS DA API
    const handleBuscarAusencias = async (e) => {
        e.preventDefault();
        if (!obraSelecionada || !dataInicio || !dataFim) {
            alert("Preencha todos os campos obrigatórios (Obra, Início e Fim).");
            return;
        }
        ...
        try {
            ...
            calendario.forEach(diaCal => {
                ...
                let tipoFiltro = 'VERMELHO'; // Padrão: Nenhuma equipe escalada
                let equipesAlocadasSemRdo = [];
                let equipesFechadasComSucesso = [];

                if (diaBanco) {
                    const alocadas = diaBanco.equipes_alocadas_sem_rdo || [];
                    const fechadas = diaBanco.equipes_fechadas_com_sucesso || [];

                    // Filtramos as equipes alocadas removendo as que já fecharam RDO
                    equipesAlocadasSemRdo = alocadas.filter(eq => !fechadas.includes(eq));
                    equipesFechadasComSucesso = fechadas;
    ...
    // FILTRAGEM: Oculta os dias 100% resolvidos (VERDE) e mantém apenas os PENDENTES (AMARELO / VERMELHO)
    const diasComPendencias = diasAuditados.filter(item => item.tipoFiltro !== 'VERDE');
    ...
    return (
        <div style={{ padding: '20px', fontFamily: 'sans-serif', backgroundColor: '#f8fafc', minHeight: '80vh' }}>
            ...
            <form onSubmit={handleBuscarAusencias} style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', backgroundColor: '#fff', padding: '15px', borderRadius: '6px', border: '1px solid #e2e8f0', marginBottom: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                ...
                {/* FILTRO DE GESTOR (APENAS PARA CARGO MASTER) */}
                {isMaster && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', minWidth: '200px', flex: 1 }}>
                        <label style={{ fontSize: '11px', fontWeight: 'bold', color: '#0284c7' }}>GESTOR (MASTER)</label>
                        <select 
                            value={gestorSelecionado} 
                            onChange={e => {
                                setGestorSelecionado(e.target.value);
                                setObraSelecionada(''); // Limpa a obra ao trocar de gestor
                            }} 
                            style={{ height: '34px', borderRadius: '4px', border: '1px solid #38bdf8', fontSize: '12px', padding: '0 8px', backgroundColor: '#f0f9ff' }}
                        >
                            <option value="">-- Todos os Gestores --</option>
                            {gestoresDisponiveis.map(g => (
                                <option key={g.id} value={g.id}>{g.nome || g.usuario}</option>
                            ))}
                        </select>
                    </div>
                )}
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                    <button type="submit" disabled={loading} style={{ height: '34px', padding: '0 16px', backgroundColor: '#0284c7', color: '#fff', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', fontSize: '12px' }}>
                        {loading ? 'Processando...' : 'Auditar Pendências'}
                    </button>
                </div>
            </form>

            {/* RENDERIZAÇÃO DOS CARDS DO CALENDÁRIO (APENAS PENDENTES) */}
            {buscaRealizada && (
                <div style={{ backgroundColor: '#fff', padding: '15px', borderRadius: '6px', border: '1px solid #cbd5e1', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                    
                    {diasComPendencias.length === 0 ? (
                        <div style={{ padding: '20px', textAlign: 'center', color: '#16a34a', fontWeight: 'bold', backgroundColor: '#f0fdf4', borderRadius: '4px', border: '1px solid #bbf7d0' }}>
                            🎉 Nenhuma pendência encontrada para o período e obra selecionados! Todos os RDOs foram concluídos.
                        </div>
                    ) : (
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '10px' }}>
                            {diasComPendencias.map((item) => {
